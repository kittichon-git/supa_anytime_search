...
def main():
    ...
    for group, q, tbs, pages in queries:
        ...
        for item in hits:
            ...
            # --- ดึง Domain ไปเก็บใน crawler_domains_normal ---
            try:
                ...
                if hostname.endswith((".go.th", ".ac.th", ".or.th")):
                    if hostname not in seen_domains:
                        ...
                        domain_rows.append({
                            "domain": hostname,
                            "index_url": f"{parsed_url.scheme}://{hostname}/",
                            "url_pattern": "/",
                            "keyword_found": "serper daily",
        ...
        if domain_rows:
            try:
                sb.table(DOMAIN_TABLE).upsert(
                    domain_rows, on_conflict="domain", ignore_duplicates=True
                ).execute()
            except Exception as e:
                log.warning(f"Domain upsert error: {e}")
